...
function eliminar(idImagenes){
                    Swal.fire({
                        ...
                        if (result.value) {
                            $.post('../../controllers/Imagenes.controllers.php?op=eliminar',{idImagenes:idImagenes}, function (data) {
...
var cargaTabla = () => {
                    ...
                    $.post('../../controllers/Imagenes.controllers.php?op=todos', (datos) => {
                       ...
                       $.each(datos, (index, val) => {
                           html += '<tr><td>' + (index + 1) + '</td><td>'+ val.Opciones_Respuestas_idOpciones_Respuestas+'</td><td>'+ val.Preguntas_idPreguntas+'</td><td>'+ val.TituloImagen+'</td><td>'+ val.URLImagen+'</td><td>'+ val.DescripcionImagen+'</td>';if((Usuario_IdRoles) === "1") {html +='<td><button type=button; onClick=editar('+ val.idImagenes+') class=btn&#32;btn-outline-success>Editar</button>'}  if ((Usuario_IdRoles) === "1") {html += '<button type=button onClick=eliminar(' + val.idImagenes + ') class=btn&#32;btn-outline-danger&#32;btn-icon>Eliminar</button></td></tr>';} if ((Usuario_IdRoles) === "2" || (Usuario_IdRoles) === "2") {html +='<td><button type=button onClick=editar('+ val.idImagenes+') class=btn&#32;btn-outline-success>Editar</button></tr>';}
                        });
...
var editar = (idImagenes) => {
                    ...
                    jQuery.post('../../controllers/Imagenes.controllers.php?op=uno', {
                        idImagenes: idImagenes
                    }, (unImagenes) => {
                        unImagenes = JSON.parse(unImagenes);
document.getElementById('idImagenes').value = unImagenes.idImagenes;
document.getElementById('Opciones_Respuestas_idOpciones_Respuestas').value = unImagenes.Opciones_Respuestas_idOpciones_Respuestas;
document.getElementById('Preguntas_idPreguntas').value = unImagenes.Preguntas_idPreguntas;
...
document.getElementById('DescripcionImagen').value = unImagenes.DescripcionImagen;
 document.getElementById('titleLabelImagenes').innerHTML = 'Editar Usuario';
...
var nuevo = () => {
nuevoOpciones();nuevoPreguntas();  document.getElementById('titleLabelImagenes').innerHTML = 'Nuevo Imagenes';};
var limpiacajas = () => {
